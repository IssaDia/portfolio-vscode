...
const Buttons = () => {
  ...
  return (
    <div
      className={`flex justify-evenly items-center w-20 cursor-pointer ${
        isHovered ? "hover" : ""
      }`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <span className="relative inline-block p-2 bg-topbar-firstButton  rounded-full h-4">
        <span
      ...
      <span className="relative inline-block p-2 bg-topbar-secondButton rounded-full h-4">
        <span
          className={` text-black ${
            isHovered ? "opacity-100" : "opacity-0"
          } hover:opacity-100 transition-opacity absolute  transform -translate-x-1/2 -translate-y-1/2`}
      ...
      <span className="relative inline-block p-2 bg-topbar-thirdButton  h-4 rounded-full">
        <span
          className={` text-black ${
            isHovered ? "opacity-100" : "opacity-0"
          } hover:opacity-100 transition-opacity absolute  transform -translate-x-1/2 -translate-y-1/2`}
        >
          <svg height="10px" viewBox="0 0 16 16" width="10px">
